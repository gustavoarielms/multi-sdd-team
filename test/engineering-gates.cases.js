...
test("engineering gate configuration is strict and requires the exact executor allowlist", async () => {
  ...
  assert.equal(inventory.length, 116);
  assert.equal(new Set(inventory).size, 116);
  assert.equal(inventory.filter((name) => classifyTestName(name) === "unit").length, 18);
  assert.equal(inventory.filter((name) => classifyTestName(name) === "integration").length, 98);
